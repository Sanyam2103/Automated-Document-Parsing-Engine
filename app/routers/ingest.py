...
async def ingest_documents_v2(request: IngestRequestV2):
    ...
    for doc in request.documents:
        ...
        redacted_text, pii_hashes = PIIRedactor.redact_and_hash_pii(doc.text)

        # Store both redacted text AND hashes for verification
        redacted_docs.append({
            "name": doc.name or f"document_{len(redacted_docs)}",
            "type": doc_type,
...
async def analyze_documents(request_id: Optional[str]):
    ...
    try:
        ...
        if rag_service and llm_service:
            # Full AI analysis
            checklist = rag_service.build_policy_checklist(parsed_data)
            brief = llm_service.generate_negotiation_brief(parsed_data, checklist)
            client_email = llm_service.generate_client_email(parsed_data, checklist)
            ...
            return {
                ...
                "powered_by": "Google Gemini 1.5 Flash + RAG"
            }
        else:
